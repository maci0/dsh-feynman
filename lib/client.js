...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function KeyRow(props) {
      const { t, remote, label, ref, state, onRefresh } = props
      ...
      return React.createElement(
        ...
        React.createElement('span', { className: 'rk-badge' }, `${ref}: ${word(t, state)}`),
        React.createElement(
          'div',
          { className: 'rk-row' },
          ...
          React.createElement(
            'button',
            {
              ...
              onClick: () => { void run(() => remote.credentials.set(ref, staged)) },
            },
            t('save'),
          ),
          state.configured
            ? React.createElement(
              'button',
              {
                ...
                className: 'rk-btn',
                disabled: busy || !state.writable,
                onClick: () => { void run(() => remote.credentials.unset(ref)) },
              },
              t('clear'),
            )
            : null,
      ...
      const Card = function ResearchKeysCard() {
        ...
        const refresh = async () => {
          const entries = await Promise.all(Object.entries(SLOTS).map(async ([id, refField]) => {
            ...
            const view = response?.ok ? response.value[refs[refField]] : undefined
            return [id, { configured: view?.configured ?? false, writable: view?.writable ?? true }]
          }))
